...
const std::vector<std::string> keywords{
    ...
    "float", "double",   "union",  "enum", "struct",   "const",
    "if",    "else",     "switch", "case", "for",      "while",
    "do",    "continue", "break",  "goto", "return"};
...
bool is_(char c) { return c == '_'; }

bool isSpecialChars(char c) {
    return std::find(specialChars.begin(), specialChars.end(), c) !=
           specialChars.end();
}
...
class Lexer {
   ...
   public:
    ...
    // Token matchArithmaticToken(char c); //mainly for binary ops, relation
    // ops, arithmatic ops etc
    Token matchSpecialToken(char c);  // maily for tokens like + - * / { } etc
    Token matchIdentifierToken(char c);  // mainly for ID and keywords
    Token matchConstantToken(char c);  // mainly for number and literal
                                       // constants
...
int Lexer::getLineno() const { return this->lineno_; }

int Lexer::getCol() const { return this->col_; }
    ...
    *c = *current_;
    ...
    if (*c == EOF) {
        ...
        if (currentBuf_ == 1) {
            ...
            if (ret <= 0) {
                fprintf(stderr, "%s\n",
                        "in getNextChar 1:read error or nothing to read from "
                        "stream to buf2\n");
                return ret;
            } else {
                // set the flag
            ...
            if (ret <= 0) {
                fprintf(stderr, "%s\n",
                        "in getNextChar 2:read error or nothing to read from "
                        "stream to buf1\n");
                return ret;
            } else {
                buf1_[ret] = EOF;
...
Token Lexer::matchSpecialToken(char c) {
    ...
    tk.col_ = col_;
    switch (c) {
        // actually if we combine the + , - , * ,/ to arithmatic ops, the judge
        // is still neccesary
...
Token Lexer::matchConstantToken(char c) {
    ...
    if (c == '\'') {
        ...
        if (ret <= 0) {
            fprintf(stderr,
                    "end scanning input stream or error happens at line:%d "
                    "col:%d\n",
                    lineno_, col_);
            return tk;
        }
        ...
        if (ret <= 0 || c != '\'') {
            fprintf(stderr,
                    "the char is not expressed correctly at line:%d col:%d\n",
                    lineno_, col_);
            return tk;
        }
        temp.push_back(cr);
