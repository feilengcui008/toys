...
int main(int argc, char* argv[])
{
  ...
  if (myrank == 0) {
    ...
    for (n = 1; n < nprocs; n++) {
      MPI_Send(A + n * block_col_size * block_row_size, block_row_size * block_col_size, MPI_INT, n, 1, MPI_COMM_WORLD);
    }
    // receive results from other processes
    for (n = 1; n < nprocs; n++) {
      MPI_Recv(C + n * block_row_size * col_size2, block_row_size * col_size2, MPI_INT, n, 3, MPI_COMM_WORLD, &status);
    }
    // calculate my part
    ...
    MPI_Recv(T, row_size2 * col_size2, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
    MPI_Recv(buf, block_row_size * block_col_size, MPI_INT, 0, 1, MPI_COMM_WORLD, &status);
    ...
    MPI_Send(ans, block_row_size * col_size2, MPI_INT, 0, 3, MPI_COMM_WORLD);
  }
