...
class Command(object):
    ...
    def Execute(self):
        ...
        cmd = None
        if parserName != self.name:
            cmd = self._getParser(parserName)
            if cmd == None:
                raise CommandException("not found match command")
        else:
            cmd = self
        if cmd.run:
            cmd.run(cmd, args)
    ...
    def AddCommand(self, cmd, name=None):
        ...
        for command in self.commands:
            if command.name == cmd.name:
                raise CommandException("subcommand with the same name already exists")
        if not self.subparsers:
            self.subparsers = self.parser.add_subparsers(dest="_lastMatchedParser")
        ...
        if self.subparsers._name_parser_map.has_key(cmdName):
            raise CommandException("subcommand of name %s already exists" % (cmdName,))
