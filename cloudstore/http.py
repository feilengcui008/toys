...
""" this file contains http provider interface and its urllib2 and requests implementation """
...
class Urllib2HttpProvider(AbstractHttpProvider):
    ...
    @staticmethod
    def get(url, headers=dict(), data=None):
    ...
    @staticmethod
    def post(url, headers=dict(), data=None, files=None):
        if files is not None:
            raise HttpProviderException(
                    "urllib2 implementation does not support multipart file uploading")
        if data is not None and isinstance(data, dict):
    ...
    @staticmethod
    def put(url, headers=dict(), data=None, files=None):
        if files is not None:
            raise HttpProviderException("urllib2 implementation does not support file uploading")
        if data is not None and isinstance(data, dict):
            data = urllib.urlencode(data)
        req = urllib2.Request(url, headers=headers)
    ...
    @staticmethod
    def patch(url, headers=dict(), data=None):
        if data is not None and isinstance(data, dict):
            data = urllib.urlencode(data)
        req = urllib2.Request(url, headers=headers)
    ...
    @staticmethod
    def delete(url, headers=dict(), data=None):
        req = urllib2.Request(url, headers=headers)
...
class RequestsHttpProvider(AbstractHttpProvider):
    ...
    def delete(self, url, headers=dict()):
        """ delete method """
